...
interface NextActionsListDao {
    ...
    @Insert
    fun insert(word: NextActionsNoteDb)

    @Query("DELETE FROM NextActionsNoteDb WHERE id = :id")
    fun deleteById(id: Int)

    @Query("UPDATE NextActionsNoteDb SET name=:name, content=:content, creationDate=:creationDate, updateDate=:updateDate WHERE id = :id")
